...
import { isDev, isTest } from '../utils/mode'

const globalForPrisma = globalThis as unknown as { prisma: PrismaClient }
const dbPath =
  isDev() || isTest() || process.env.npm_command == 'start'
    ? 'dev.db'
    : path.join(process.resourcesPath, 'database/prod.db')
      ...
      query: {
        $allModels: {
          ...
          $allOperations({ model, operation, args, query }) {
            ...
            if (operation === 'upsert') {
              if (
                !('created' in args.create) ||
                !('updated' in args.create) ||
                !('updated' in args.update)
              ) {
